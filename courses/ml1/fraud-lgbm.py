...
def DO(frm,to,fileno):
    ...
    train_df = pd.read_csv("data/fraud-detection/train.csv", parse_dates=['click_time'], skiprows=range(1,frm), nrows=to-frm, dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])

    print('loading test data...')
    if debug:
        test_df = pd.read_csv("data/fraud-detection/test.csv", nrows=100000, parse_dates=['click_time'], dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'click_id'])
    else:
        test_df = pd.read_csv("data/fraud-detection/test.csv", parse_dates=['click_time'], dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'click_id'])
    ...
    train_df = do_click_shift( train_df, ['ip'], 'ip_click_time_minus')
    train_df = do_click_shift( train_df, ['ip'], 'ip_click_time_plus', shift_val=+1)
    ...
    predictors.extend(['app','device','os', 'channel', 'hour', 'day', 
                  ...
                  'X0', 'X1', 'X2', 'X3', 'X4', 'X5', 'X6', 'X7', 'X8', 'ip_click_time_minus', 'ip_click_time_plus'])
